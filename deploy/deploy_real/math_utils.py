...
# @torch.jit.script
#   require new name: multiply_transform
...
def safe_rotvec2quat(rotvec: torch.Tensor, form: Literal["xyzw", "wxyz"] = "wxyz"):
    angle = torch.linalg.vector_norm(rotvec, dim=1)
    small_angle = angle <= 1e-3
    large_angle = ~small_angle
    scale = torch.empty_like(angle)
    scale[small_angle] = (
        0.5 - angle[small_angle] ** 2 / 48 + angle[small_angle] ** 4 / 3840
    )
...
def slerp(q0: torch.Tensor, q1: torch.Tensor, steps: torch.Tensor):
